export const siteData = {
  company: {
    name: "Consol Travels",
    slogan: "Sua próxima viagem começa aqui.",
  },

  contact: {
    whatsapp: "5500000000000",
    email: "contato@consoltravels.com.br",
    instagram: "@consoltravels",
  },

  hero: {
    title: "Sua próxima viagem começa aqui",
    subtitle:
      "Planejamos viagens nacionais e internacionais com atendimento personalizado, cuidando de cada detalhe para que você viaje com tranquilidade.",
  },

  services: [
    {
      icon: "Plane",
      title: "Passagens Aéreas",
      description:
        "As melhores opções para destinos nacionais e internacionais.",
    },
    {
      icon: "Hotel",
      title: "Hospedagem",
      description:
        "Hotéis, resorts e pousadas cuidadosamente selecionados.",
    },
    {
    icon: "MapPinned",
    title: "Roteiros Personalizados",
    description:
        "Planejamento completo de acordo com seu perfil e objetivos de viagem.",
    },
    {
      icon: "Car",
      title: "Aluguel de Veículos",
      description:
        "Mais liberdade para explorar cada destino.",
    },
    {
      icon: "ShieldCheck",
      title: "Seguro Viagem",
      description:
        "Proteção e tranquilidade durante toda a viagem.",
    },
    {
      icon: "Ship",
      title: "Cruzeiros",
      description:
        "Experiências inesquecíveis pelos melhores roteiros.",
    },
  ],

  destinations: [
  {
    country: "Portugal",
    city: "Lisboa",
    image: "/images/destinations/portugal.jpg",

    credit: {
      photographer: "Dmitry Voronov",
      photographerUrl:
        "https://unsplash.com/@onthecrow?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
      photoUrl:
        "https://unsplash.com/photos/the-sun-is-setting-behind-a-bridge-over-water-FRqxfw-rSro?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
    },
    description:
        "Descubra Lisboa, uma cidade encantadora que combina história, cultura, gastronomia e paisagens inesquecíveis.",
    highlights: [
        "Centro histórico e bairros tradicionais",
        "Torre de Belém e Mosteiro dos Jerónimos",
        "Gastronomia e cultura portuguesa",
    ],
  },

  {
    country: "Estados Unidos",
    city: "Nova York",
    image: "/images/destinations/new-york.jpg",

    credit: {
      photographer: "Luca Bravo",
      photographerUrl:
        "https://unsplash.com/@lucabravo?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
      photoUrl:
        "https://unsplash.com/photos/time-square-new-york-during-daytime-TaCk3NspYe0?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
    },
    description:
        "Viva a energia de Nova York, com seus arranha-céus, cultura, gastronomia e atrações mundialmente famosas.",
    highlights: [
        "Times Square e Manhattan",
        "Central Park e principais atrações",
        "Gastronomia, compras e entretenimento",
    ],
  },

  {
    country: "França",
    city: "Paris",
    image: "/images/destinations/paris.jpg",

    credit: {
      photographer: "Timelab",
      photographerUrl:
        "https://unsplash.com/@timelabpro?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
      photoUrl:
        "https://unsplash.com/photos/the-eiffel-tower-towering-over-the-city-of-paris-uYrACAHq6jI?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
    },
    description:
        "Explore Paris, uma cidade repleta de história, arte, gastronomia e alguns dos monumentos mais conhecidos do mundo.",
    highlights: [
        "Torre Eiffel e principais monumentos",
        "Museus, arte e cultura",
        "Gastronomia e experiências parisienses",
    ],
  },

  {
    country: "Japão",
    city: "Tóquio",
    image: "/images/destinations/tokyo.jpg",

    credit: {
      photographer: "David Edelstein",
      photographerUrl:
        "https://unsplash.com/@jlhopes?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
      photoUrl:
        "https://unsplash.com/photos/mount-fuji-japan-N4DbvTUDikw?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
    },
    description:
        "Conheça Tóquio, onde tradição e tecnologia se encontram em uma das cidades mais fascinantes do mundo.",
    highlights: [
        "Templos e cultura tradicional japonesa",
        "Tecnologia e bairros modernos",
        "Gastronomia e experiências únicas",
    ],
  },

  {
    country: "Itália",
    city: "Roma",
    image: "/images/destinations/rome.jpg",

    credit: {
      photographer: "Kristian Hjuler",
      photographerUrl:
        "https://unsplash.com/@khjuler?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
      photoUrl:
        "https://unsplash.com/photos/a-city-with-many-buildings-Zcyl-HLP5IY?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
    },
    description:
        "Viaje por Roma e descubra séculos de história, arquitetura, cultura e sabores da tradicional culinária italiana.",
    highlights: [
        "Coliseu e Fórum Romano",
        "Vaticano e patrimônio histórico",
        "Gastronomia e cultura italiana",
    ],
  },

  {
    country: "Brasil",
    city: "Fernando de Noronha",
    image: "/images/destinations/noronha.jpg",

    credit: {
      photographer: "Ze Paulo",
      photographerUrl:
        "https://unsplash.com/@euzepaulo?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
      photoUrl:
        "https://unsplash.com/photos/brown-and-green-rock-formation-on-blue-sea-under-white-clouds-and-blue-sky-during-daytime-h-UU2PnN5a0?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
    },
    description:
        "Conheça Fernando de Noronha, um paraíso brasileiro com praias paradisíacas, natureza preservada e águas cristalinas.",
    highlights: [
        "Praias paradisíacas e águas cristalinas",
        "Mergulho e vida marinha",
        "Natureza preservada e trilhas",
    ],
  },
],
    advantages: [
    {
        title: "Atendimento Personalizado",
        description:
        "Cada viagem é planejada de acordo com o perfil e os objetivos do cliente.",
        icon: "Users",
    },
    {
        title: "Segurança em Cada Etapa",
        description:
        "Acompanhamos sua viagem desde o planejamento até o retorno.",
        icon: "ShieldCheck",
    },
    {
        title: "Especialistas em Destinos",
        description:
        "Conhecimento dos principais destinos nacionais e internacionais.",
        icon: "Globe2",
    },
    {
        title: "Suporte Completo",
        description:
        "Estamos disponíveis para ajudar antes, durante e depois da viagem.",
        icon: "Headset",
    },
    ],
    about: {
    eyebrow: "Sobre nós",
    title: "Muito mais do que vender viagens",

    description:
        "Na Consol Travels acreditamos que viajar é viver novas experiências, conhecer culturas e criar memórias para toda a vida. Por isso planejamos cada detalhe da sua viagem com dedicação, segurança e atendimento personalizado.",

    image: "/images/about/travel-planning.jpg",

    credit: {
        photographer: "Charlotte Noelle",
        photographerUrl:
        "https://unsplash.com/@charlottenoelle_?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
        photoUrl:
        "https://unsplash.com/photos/black-dslr-camera-near-passport-98WPMlTl5xo?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText",
    },

    highlights: [
        "Atendimento próximo e personalizado",
        "Planejamento completo do roteiro",
        "Suporte antes, durante e após a viagem",
        "Experiências planejadas para cada perfil de viajante",
        "Parcerias com fornecedores confiáveis",
    ],
    },
    contactSection: {
    eyebrow: "Fale conosco",

    title: "Vamos planejar sua próxima viagem?",

    description:
        "Entre em contato conosco pelo WhatsApp, e-mail ou Instagram. Será um prazer ajudar você a transformar sua próxima viagem em uma experiência inesquecível.",
    },
};